...
switch ($_POST['action']) {
	case 'admin':
		...
		if ($_POST['record'] == 0) {
			$statement = "INSERT INTO rb_ex_dati_amministrativi_esame (anno, presidente, vice, segretario) 
						  VALUES ({$y}, '{$presidente}', {$vice}, {$segretario})";
		}
		else {
			$statement = "UPDATE rb_ex_dati_amministrativi_esame 
	...
	case 'oral':
		$comm = $_POST['comm'];
		$date = format_date($_POST['val'], IT_DATE_STYLE, SQL_DATE_STYLE, "-");
		$statement = "REPLACE INTO rb_ex_orali (id_commissione, anno, data) 
						  VALUES ({$comm}, {$y}, '{$date}')";
		try {
			$id = $db->executeUpdate($statement);
			$response['id'] = $id;
