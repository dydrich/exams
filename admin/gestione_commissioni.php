...
switch ($_POST['action']) {
	case 'create_records':
		...
		while ($r = $res_cls->fetch_assoc()) {
			$idc = $db->executeUpdate("INSERT INTO rb_ex_commissioni_esame (numero, classe, sezione, anno) 
			...
			while ($row = $res_comm->fetch_assoc()) {
				$db->executeUpdate("INSERT INTO rb_ex_docenti_commissione_esame (commissione, docente, sostituto) VALUES ($idc, {$row['uid']}, NULL)");
			}
	...
	case 'substitute':
		$user = $_POST['user'];
		$substitute = $_POST['sub'];
		$comm = $_POST['comm'];
		$row_id = $db->executeUpdate("UPDATE rb_ex_docenti_commissione_esame SET sostituto = {$substitute} WHERE docente = {$user} AND commissione = {$comm}");
		break;
	case 'del_sub':
		$user = $_POST['user'];
		$comm = $_POST['comm'];
		$db->executeUpdate("UPDATE rb_ex_docenti_commissione_esame SET sostituto = NULL WHERE docente = {$user} AND commissione = {$comm}");
		break;
}
